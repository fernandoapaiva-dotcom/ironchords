...
const targetRootDelete = '{/* Rename List Modal */}'; // We will place it right before this
let deleteModalIdx = content.indexOf(startDeleteModal);

if (deleteModalIdx !== -1) {
    let deleteModalEnd = content.indexOf(')', deleteModalIdx + startDeleteModal.length);
    // Find the actual end of the modal by counting braces if needed, but we can look for the closing div
    // The Delete modal is wrapped in `{ deleteModalOpen && ( ... )}`
    let afterCondition = content.indexOf('(', deleteModalIdx);
    let closeIdx = content.indexOf(')}', deleteModalIdx);
    if (closeIdx !== -1) {
        let blockStr = content.substring(deleteModalIdx, closeIdx + 2);
        if (blockStr.includes('deleteModalOpen')) {
            content = content.replace(blockStr, '');
            let targetIdx = content.indexOf(targetRootDelete);
            if (targetIdx !== -1) {
                content = content.slice(0, targetIdx) + blockStr + '\n\n            ' + content.slice(targetIdx);
                changes++;
...
if (saveModalIdx !== -1) {
    // Save modal is wrapped in `{ saveListModalOpen && (() => { ... })() }`
    let closeIdx = content.indexOf('})()', saveModalIdx);
    if (closeIdx !== -1) {
        // Needs the closing `}`
        let blockEnd = content.indexOf('}', closeIdx) + 1;
        let blockStr = content.substring(saveModalIdx, blockEnd);
        if (blockStr.includes('saveListModalOpen')) {
            content = content.replace(blockStr, '');
            let targetIdx = content.indexOf(targetRootDelete);
            if (targetIdx !== -1) {
                content = content.slice(0, targetIdx) + blockStr + '\n\n            ' + content.slice(targetIdx);
...
// === 3. Extract Batch Upload Logic ===
// We need to wrap it in a modal over the root.
// The batch logic starts at `<div className="flex flex-col items-center justify-center py-10 space-y-8 animate-in fade-in duration-500 relative">`
// Wait, it is inside the `listasSubTab === 'lote'` branch.
const batchStartMarker = 'listasSubTab === \'lote\' ? (';
let batchStartIdx = content.indexOf(batchStartMarker);

if (batchStartIdx !== -1) {
    let nextBranch = content.indexOf(') : (', batchStartIdx); // wait, it was inside `listasSubTab === 'salvas' ? ( ... ) : ( ... )`

    // Look for `<div className="flex flex-col items-center justify-center py-10...` which is the start of the batch UI
    let actualBatchUiStart = content.indexOf('<div className="flex flex-col items-center justify-center py-10', batchStartIdx);

    if (actualBatchUiStart !== -1) {
        // Finding the end of this div is tricky, but we know it usually ends before `</div>\n                                        </div>\n                                    </div>\n                                );`
        // Let's use regex to grab the whole `listasSubTab === 'lote'` branch.
        const startString = `<div className="flex flex-col items-center justify-center py-10 space-y-8 animate-in fade-in duration-500 relative">`;
        ...
        if (batchUiStartIdx !== -1) {
            ...
            for (let i = 0; i < tempStr.length; i++) {
                if (tempStr.substring(i, i + 4) === '<div') {
                    depth++;
            ...
            if (batchUiEndIdx !== -1) {
                ...
                content = content.replace(batchBlock, '<div className="hidden">Batch UI Removed</div>'); // remove it from dead code

                // Construct the Batch Modal
                let batchModal = `
            {/* Batch Upload Modal */}
            {batchModalOpen && (
                ...
                let targetIdx = content.indexOf(targetRootDelete);
                if (targetIdx !== -1) {
                    content = content.slice(0, targetIdx) + batchModal + '\n\n            ' + content.slice(targetIdx);
                    changes++;
                    console.log('✅ Extracted Batch Upload to Modal.');
                }
            }
        }
    }
...
// 4. Clean up `listasSubTab` from Listas sidebar and add `Lote` trigger button to the Listas sidebar
...
if (searchBarIdx !== -1) {
    let parentStartIdx = content.lastIndexOf('<div className="relative mb-3">', searchBarIdx); // Note: Original was relative mb-3 without flex-shrink
    if (parentStartIdx === -1) parentStartIdx = content.lastIndexOf('<div className="relative mb-3', searchBarIdx);
