...
def process_chords(content: str, from_key: str, to_key: str) -> str:
    ...
    # regex for chords like C#m7, G/B, etc
    # We use lookarounds to ensure the chord is not part of a larger word
    # while allowing characters like '#' which \b wouldn't handle correctly.
    chord_regex = re.compile(r"(?<![a-zA-Z0-9])(?P<chord>[A-G][b#]?(?:m|maj|min|dim|aug|sus)?(?:\d)?(?:/[A-G][b#]?)?)(?![a-zA-Z0-9])")
